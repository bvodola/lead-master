...
class ClientScreen extends React.Component {
  ...
  render() {
    ...
    const sameDayAsNext = (t, i) =>
      t[i + 1] &&
      moment(t[i].tDate).format('YY-MM-DDDD') === moment(t[i + 1].tDate).format('YY-MM-DDDD');
    const sameDayAsBefore = (t, i) =>
      t[i - 1] &&
      moment(t[i].tDate).format('YY-MM-DDDD') === moment(t[i - 1].tDate).format('YY-MM-DDDD');
      ...
      <WithData query={CLIENT({ _id: clientId })}>
        {({ data, mutation, handler }) => {
          ...
          tasks = tasks.map(t => ({
            ...t,
            type: 'task',
            tDate: new Date(t.date + 'T23:59:59.999999-02:00')
          }));
          ...
          return (
            <div>
                ...
                {timeline.map((item, i) => (
                  <Tr
                    ...
                    style={{
                      ...
                      backgroundColor:
                        item._id === this.state.selectedTableRow ? '#eee' : 'transparent',
                      ':hover': {
                        backgroundColor: '#eee'
                      }
                    }}
                      ...
                      {item.type === 'task' ? (
                        <DatePickerButton
                          ...
                          onChange={date => {
                            ...
                            handler.set(
                              'data.client.tasks',
                              data.client.tasks.map(t =>
                                t._id === item._id ? { ...item, date } : t
                              )
                            );
                        ...
                        <Checkbox
                          ...
                          onClick={() => {
                            handler.set(
                              'data.client.tasks',
                              data.client.tasks.map(t =>
                                t._id === item._id ? { ...item, completed: !item.completed } : t
                              )
                            );
                      ...
                      <ContentEditable
                        ...
                        onChange={ev => {
                          ...
                          handler.set(
                            ...
                            data.client[`${item.type}s`].map(t =>
                              t._id === item._id ? { ...item, content } : t
                            )
                          );
                          item.type == 'task'
              ...
              <Modal open={this.state.addTaskModalOpen} toggle={this.toggleAddTaskModal}>
              ...
              <Modal open={this.state.addLogModalOpen} toggle={this.toggleAddLogModal}>
